...
export type Tab = 'catalog' | 'requirements' | 'technologies' | 'domains' | 'techdomains' | 'solutions' | 'architectures';
...
export type SolutionView = 'list' | 'detail' | 'create' | 'edit';
export type SolutionStatus = 'draft' | 'review' | 'approved' | 'rejected' | 'archived';
export type ArchView = 'list' | 'detail' | 'create' | 'edit';
export type ArchStatus = 'draft' | 'review' | 'approved' | 'rejected' | 'archived';
...
export interface TypicalArchitecture {
  id: string;
  name: string;
  description: string;
  status: ArchStatus;
  ...
  schemes: MermaidScheme[];
  solutionIds: string[];
  approvedByIb: boolean;
  approvedByIt: boolean;
  createdAt: string;
  updatedAt: string;
}

export const ARCH_STATUS_CONFIG: Record<ArchStatus, { label: string; color: string }> = {
  draft:    { label: 'Черновик',   color: 'text-slate-400 bg-slate-400/10 border-slate-400/30' },
  review:   { label: 'На ревью',   color: 'text-cyan-400 bg-cyan-400/10 border-cyan-400/30' },
  approved: { label: 'Утверждено', color: 'text-green-400 bg-green-400/10 border-green-400/30' },
  rejected: { label: 'Отклонено', color: 'text-red-400 bg-red-400/10 border-red-400/30' },
...
export const emptyArchForm = {
  name: '', description: '', status: 'draft' as ArchStatus,
  author: '', techDomainId: '', version: '1.0',
  tags: [] as string[], schemes: [] as MermaidScheme[],
  solutionIds: [] as string[],
  approvedByIb: false, approvedByIt: false,
};

export const MOCK_ARCHITECTURES: TypicalArchitecture[] = [
  {
    id: 'ARCH-001', name: 'Микросервисная авторизация', version: '2.1',
    author: 'Алексей Петров', techDomainId: 'TDOM-001',
    status: 'approved',
    description: 'Типовая архитектура сервиса авторизации на базе Keycloak с поддержкой OAuth 2.0, LDAP и SSO для микросервисных систем.',
    tags: ['auth', 'microservices', 'keycloak', 'SSO'],
    schemes: [],
    solutionIds: ['SOL-001', 'SOL-002'],
    approvedByIb: true, approvedByIt: true,
    createdAt: '2024-01-20', updatedAt: '2024-02-28',
  },
  {
    id: 'ARCH-002', name: 'API Gateway Pattern', version: '1.0',
    author: 'Мария Иванова', techDomainId: 'TDOM-002',
    status: 'review',
    description: 'Типовое решение для централизованного управления входящим трафиком с балансировкой нагрузки через Kong и NGINX.',
    tags: ['gateway', 'nginx', 'kong', 'proxy'],
    schemes: [],
    solutionIds: ['SOL-001'],
    approvedByIb: false, approvedByIt: true,
    createdAt: '2024-02-01', updatedAt: '2024-03-01',
  },
];

export const SOLUTION_STATUS_CONFIG: Record<SolutionStatus, { label: string; color: string }> = {
  draft:    { label: 'Черновик',  color: 'text-slate-400 bg-slate-400/10 border-slate-400/30' },
  review:   { label: 'На ревью', color: 'text-cyan-400 bg-cyan-400/10 border-cyan-400/30' },
  approved: { label: 'Утверждено', color: 'text-green-400 bg-green-400/10 border-green-400/30' },
  rejected: { label: 'Отклонено', color: 'text-red-400 bg-red-400/10 border-red-400/30' },
...
export const emptySolutionForm = {
  name: '', version: '1.0', owner: '', author: '', status: 'draft' as SolutionStatus, description: '', tags: [] as string[], technologyIds: [] as string[],
};
...
export const MOCK_SOLUTIONS: TechnicalSolution[] = [
  {
    id: 'SOL-001', name: 'NGINX', version: '1.25.3', owner: 'Дмитрий Орлов', author: 'Игорь Сысоев',
    status: 'approved',
    description: 'Nginx (Engine X) — веб-сервер с открытым исходным кодом. Может выступать как классический веб-сервер, реверс-прокси и балансировщик нагрузки.',
    tags: ['WEB', 'Proxy', 'LoadBalancer'],
    technologyIds: ['TECH-001', 'TECH-002'],
    createdAt: '2024-01-15', updatedAt: '2024-02-20',
  },
  {
    id: 'SOL-002', name: 'Сервис аутентификации', version: '2.0.0', owner: 'Мария Иванова', author: 'Алексей Петров',
    status: 'review',
    description: 'Сервис единой точки аутентификации на базе Keycloak с поддержкой SSO, OAuth 2.0 и LDAP-интеграцией.',
    tags: ['Auth', 'SSO', 'OAuth'],
